...
if (!token)
  throw new Error("No Square account token in dev.db, sign in as test1 first");
...
const objects = [
  item("#espresso", "Espresso", "Double shot", [
    variation("#espresso-regular", "#espresso", "Regular", 300),
  ]),
  item("#latte", "Latte", "Espresso + steamed milk", [
    variation("#latte-small", "#latte", "Small", 400),
    variation("#latte-large", "#latte", "Large", 500),
  ]),
  item("#cappuccino", "Cappuccino", "Espresso + foam", [
    variation("#cappuccino-regular", "#cappuccino", "Regular", 450),
  ]),
  item("#cold-brew", "Cold Brew", "Slow-steeped", [
    variation("#cold-brew-regular", "#cold-brew", "Regular", 425),
  ]),
  item("#croissant", "Croissant", "Butter croissant", [
    variation("#croissant-regular", "#croissant", "Regular", 350),
  ]),
...
const res = await fetch(`${BASE}/v2/catalog/batch-upsert`, {
  ...
  body: JSON.stringify({
    idempotency_key: "pos-catalog-seed-v1",
    batches: [{ objects }],
  }),
});
...
writeFileSync(
  "src/shared/square/seed-ids.json",
  `${JSON.stringify(idMap, null, 2)}\n`,
);
console.log(
  `Seeded ${json.objects?.length ?? 0} objects; wrote ${Object.keys(idMap).length} ID mappings.`,
);
